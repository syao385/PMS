...
export const RightPanel: React.FC<RightPanelProps> = ({ currentTicker }) => {
    ...
    async function loadNews() {
      ...
      if (news && news.length > 0) {
        ...
        setLiveNews([
          {
            id: 'n1',
            title: `${currentTicker}: Live Financial Coverage & Market Analysis`,
  ...
  const filteredNews = activeFeedTab === 'ALL' ? liveNews : liveNews.filter((n) => n.feed === activeFeedTab);
      ...
      <div className="glass-card p-4 space-y-3 flex flex-col max-h-[420px]">
        <div className="flex items-center justify-between border-b border-slate-800 pb-2">
          <span className="font-bold text-xs text-white uppercase tracking-wider font-mono flex items-center gap-1.5">
            ...
            NEWS PORTAL FEED ({currentTicker})
          </span>
          <span className="text-[10px] text-emerald-400 font-mono font-bold">Realtime Live Feed</span>
        </div>
